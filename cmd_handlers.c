 ...
 * Adds some proprietary command examples to retrieve version info.
 */
/*******************************************************************************
* # License
...
// Include for application and version info
...
ZW_ADD_CMD(FUNC_ID_PROPRIETARY_0) {
  /* Add application version info as proprietary cmd 0 (F0) */
  /* Will send 32-bit value little endian */
  extern const ApplicationProperties_t sl_app_properties;
  memcpy(compl_workbuf,&sl_app_properties.app.version,
           sizeof(sl_app_properties.app.version));
  DoRespond_workbuf(sizeof(sl_app_properties.app.version));
}

ZW_ADD_CMD(FUNC_ID_PROPRIETARY_1) {
  /* Add bootloader version info as proprietary cmd 1 (F1) */
  /* Will send 32-bit value little endian */

  BootloaderInformation_t bootloaderInfo;
  bootloader_getInfo(&bootloaderInfo);

  memcpy(compl_workbuf,&bootloaderInfo.version, sizeof(bootloaderInfo.version));
  DoRespond_workbuf(sizeof(bootloaderInfo.version));
}
